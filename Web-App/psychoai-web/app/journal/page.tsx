  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <div className="flex-1 ml-60 p-8 max-w-4xl animate-fade-in">
        <h1 className="font-display font-extrabold text-3xl text-gray-900 mb-1">Journal</h1>
        ...
        {mode === 'choose' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <button onClick={() => setMode('cbt')} className="w-full card p-6 flex items-center gap-4 hover:shadow-md transition-shadow text-left border-l-4 border-teal-500">
              ...
              <button onClick={() => { setMode('free'); setPrompt(FREE_PROMPTS[Math.floor(Math.random()*FREE_PROMPTS.length)]); }}
                className="w-full card p-6 flex items-center gap-4 hover:shadow-md transition-shadow text-left border-l-4 border-blue-500">
                <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center flex-shrink-0">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M12 20h9" stroke="#1A56DB" strokeWidth="1.8"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z" stroke="#1A56DB" strokeWidth="1.8"/></svg>
                </div>
                <div>
        ...
        {mode === 'cbt' && (
          <div className="max-w-2xl animate-fade-in">
            ...
            <div className="space-y-5">
              ...
              <button onClick={saveCBT} disabled={loading} className="w-full bg-teal-500 hover:bg-teal-600 text-white font-bold py-4 rounded-2xl transition-colors">
        ...
        {mode === 'free' && (
          <div className="max-w-2xl animate-fade-in">
            ...
            <div className="card p-5 mb-4 bg-blue-50 border border-blue-100">
              <p className="text-sm text-blue-700 italic">"{prompt}"</p>
              <button onClick={() => setPrompt(FREE_PROMPTS[Math.floor(Math.random()*FREE_PROMPTS.length)])} className="text-xs text-blue-500 font-medium mt-2 hover:text-blue-700">Try another prompt</button>
            </div>
            ...
            <button onClick={saveFree} disabled={!freeText.trim()||loading} className="w-full bg-teal-500 hover:bg-teal-600 disabled:opacity-40 text-white font-bold py-4 rounded-2xl transition-colors">
              {loading ? 'Saving...' : 'Save entry'}
            </button>
          </div>
        )}
